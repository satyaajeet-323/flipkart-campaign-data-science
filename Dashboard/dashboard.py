    ...
    if uploaded is not None:
        try:
            ...
            cand_sens = [
                ...
                if _tmp_df[c].dtype == 'object' and _tmp_df[c].nunique() <= 20
            ]
            if cand_sens:
    ...
    if uploaded is not None:
        try:
            ...
            if df_in.empty:
                ...
                st.write(f"Loaded {len(df_in)} campaigns")
                st.dataframe(df_in.head(), use_container_width=True)
        except Exception as e:
            st.error(f"Could not read CSV: {e}")
            df_in = None
    else:
        st.info("Upload a CSV or use sample data below.")
...
with tab_shap:
    ...
    if uploaded is None:
        st.info("Upload a CSV to analyze feature importance.")
    else:
        try:
...
with tab_fair:
    ...
    if uploaded is None:
        st.info("Upload a CSV to analyze performance across groups.")
    else:
        try:
            df = pd.read_csv(io.BytesIO(uploaded.getvalue()))
            aligned_df = align_columns(df.copy(), EXPECTED_COLS)
...
with tab_drift:
    ...
    if uploaded is None:
        st.info("Upload a CSV to check for data drift.")
    else:
        try:
...
with tab_eda:
    ...
    if uploaded is None:
        st.info("Upload a CSV to explore the data.")
    else:
        try:
            df = pd.read_csv(io.BytesIO(uploaded.getvalue()))
